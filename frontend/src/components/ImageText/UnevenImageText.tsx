...
const UnevenImageText: React.FC<ImageTextBlock> = ({ colour, imageSide, image, children }) => {
  ...
  return (
    <div css={[tw`relative rounded-xl overflow-hidden my-12`, ...getFilledColourStyles(colour)]}>
      ...
      >
        <div css={[tw`h-full w-full`, !vertical && tw`xl:(grid grid-cols-7)`]}>
          <div
            css={[
              tw`h-full relative xl:(col-span-3)`,
              imageSide === 'left' && tw`xl:col-start-1`,
              imageSide === 'right' && tw`xl:col-start-5`
            ]}
          >
            <img
              src={imgSrc}
      ...
      <div
        css={[
          tw`max-w-7xl mx-auto px-4 sm:px-6 lg:(max-w-full px-8)`,
          !vertical && tw`xl:(grid grid-cols-7 grid-flow-col-dense gap-x-12)`
        ]}
      >
        <div
          css={[
            tw`relative`,
            !vertical && tw`xl:(col-span-4 py-24)`,
            imageSide === 'top' && tw`pb-6 pt-80`,
            imageSide === 'bottom' && tw`pt-6 pb-80`,
            imageSide === 'left' && tw`pb-6 pt-80 lg:(pt-128) xl:(pt-12) xl:col-start-4`,
            imageSide === 'right' && tw`pt-6 pb-80 lg:(pb-128) xl:(pb-12) xl:col-start-1`
          ]}
        >
          {children}
        </div>
...
export default UnevenImageText;
